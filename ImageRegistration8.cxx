...
int main( int argc, char *argv[] )
{
  ...
  if( argc > SLICEBEFORE )
    ...
    resampler->SetTransform( identity );
    sliceWriter->SetFileName( argv[SLICEBEFORE] );
    sliceWriter->Update();
    }
  if( argc > SLICEDIFFBEFORE )
    {
    extractor->SetInput( intensityRescaler->GetOutput() );
    resampler->SetTransform( identity );
    sliceWriter->SetFileName( argv[SLICEDIFFBEFORE] );
    sliceWriter->Update();
    }
  if( argc > SLICEDIFFAFTER )
    {
    resampler->SetTransform( finalTransform );
    sliceWriter->SetFileName( argv[SLICEDIFFAFTER] );
    sliceWriter->Update();
    }
  if( argc > SLICEAFTER )
    {
    extractor->SetInput( caster->GetOutput() );
    resampler->SetTransform( finalTransform );
    sliceWriter->SetFileName( argv[SLICEAFTER] );
    sliceWriter->Update();
    }
  return EXIT_SUCCESS;
}
